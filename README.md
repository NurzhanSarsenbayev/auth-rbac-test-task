...
5. изменение permission через `PATCH /admin/permissions/{permission_id}`  
   (в demo используется конкретный id из seeded-данных)
...
В демонстрационном сценарии используется конкретный `permission_id` из заранее заполненных demo-данных.
Это не фиксированная константа API, а идентификатор записи в базе, который стабилен только в рамках подготовленного seed-сценария.
---
